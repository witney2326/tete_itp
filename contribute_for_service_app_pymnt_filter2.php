...
<head>
    <title><?php echo $language["Approved_Payments"]?></title>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            <h4 class="mb-sm-0 font-size-18"><?php echo $language["Approved_Payments"]?></h4>

                            <div class="page-title-right">
                                <ol class="breadcrumb m-0">
                                    <li class="breadcrumb-item"><a href="index_check.php"><?php echo $language["Dashboard"]?></a></li>
                                    <li class="breadcrumb-item active"><?php echo $language["Approved_Payments"]?></li>
                                </ol>
                            </div>
                                ...
                                <div class="tab-pane active" id="home-1" role="tabpanel">
                                    <p class="mb-0">
                                        ...
                                        <div class="card-border">
                                            <div class="card-body">
                                                <form class="row row-cols-lg-auto g-3 align-items-center" novalidate action="contribute_for_service_app_pymnt_filter3.php" method ="POST" >
                                                    <div class="col-12">
                                            ...
                                            <div class="col-12">
                                                ...
                                                <h7 class="card-title mt-0"></h7>
                                                        ...
                                                        <table id="datatable" class="table table-bordered dt-responsive  nowrap w-100">
                                                            ...
                                                            <tbody>
                                                                <?Php
                                                                    $query="select * from tpayments inner join households on tpayments.hhCode = households.hhcode where ((households.pa='$admin_post') and (households.locality='$bairro') and (households.selected_product <> '04') and (households.enrolled = '1') and (households.product_approved = '1') and (households.agree_tcs = '1') and (tpayments.pApproved = '1'))";

                                                                    
                                                                    if ($result_set = $link->query($query)) {
                                                                    while($row = $result_set->fetch_array(MYSQLI_ASSOC))
                                                                    { 
                                                                        if ($row["pApproved"] == 1){$pApproved = $language["Yes"];}else{$pApproved = $language["No"];}
                                                                        if ($row["pOption"] == '00'){$pOption = $language["None"];}else{$pOption = payment_option_name($link,$row["pOption"]);}
                                                                        if ($row["enrolled"]== 1){$enrolled = $language["Yes"];}else {$enrolled = $language["No"];}
                                                                    echo "<tr>\n";
                                                                        echo "<td>".$row["pID"]."</td>\n";
